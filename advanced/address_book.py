...
def set_contact():
    ...
    try: # 아무입력없이 엔터 | 갯수 틀리면 생기는 예외처리
        name, phone_num, e_mail, addr = input('정보입력(이름,핸드폰,이메일,주소(구분자 /)) >').split('/')
...
def save_contact(lst_contact):
    f = open('C:/DEV/STUDY/StudyPython22/advanced/db_contact.txt', mode = 'w', encoding = 'utf-8')
    for contact in lst_contact:
...
def load_contact(lst_contact):
    f = open('C:/DEV/STUDY/StudyPython22/advanced/db_contact.txt', mode = 'r', encoding = 'utf-8')
    while True:
        line = f.readline()
        if not line: break
